...
abstract: >
  TFregulome is a comprehensive transcription factor binding site (TFBS) database combining MethMotif and GTRD. The binding motifs predicted in-silico from MethMotif and GTRD describe cell specific transcription factor (TF) binding propensities, while the DNA methylation profiles from MethMotif portray a second epigenetic dimension in TF binding events. Therefore, TFregulome database is useful to study TF binding partners along with DNA methylation. TFregulomeR is an effective and efficient application programming interface (API) based R package that facilitates the retrieval and exploitation of TFregulome datasets. This package comprises functions that simplify the queries in TFregulome database. In addition, TFregulomeR uses standard objects compatible with other TFBS packages such as TFBSTools, allowing users to take advantage of the tools developed by the community. Further, the functionalities implemented in our toolbox allow to better understand the mechanism of TF recruitment in a cell-specific manner. As an illustration, we are able to identify different MAFF co-binding proteins (including the known partners), which are associated with distinct DNA binding motifs and varied methyl-CpG binding abilities across cell types, and to explore the different functions associated with MAFF different binding contexts.
...
#>     sample_type     cell_tissue_name                      description disease_state
#> 1 primary_cells embryonic-stem-cells             embryonic stem cells        normal
#> 2 primary_cells       CD4pos-T-cells                     CD4+ T-cells        normal
#> 3     cell_line               EWS502                    Ewing sarcoma         tumor
#> 4     cell_line                HUVEC umbilical vein endothelial cells        normal
#> 5     cell_line                LS180                     colon cancer         tumor
#> 6     cell_line                LS180                     colon cancer         tumor
#>       TF source source_ID peak_num peak_with_motif_num
#> 1 PRDM14   GTRD EXP000061    17482                5656
#> 2    YY1   GTRD EXP000080    18298                2038
#> 3   FLI1   GTRD EXP000128    89523               28914
#> 4   FLI1   GTRD EXP000132    71437               36126
#> 5 TCF7L2   GTRD EXP000140    16517                2312
#> 6  CEBPB   GTRD EXP000142   128542               67747
...
motif weight position matrix and its DNA methylation matrix (beta score matrix). 
...
#> [1,]   35    0    0    0    0  503    0    0    0     0    31 # beta score < 10%
#> [2,]   18    0    0    0    0  797    0    0    0     0    22 # beta score >=10% && <=90%
#> [3,]    3    0    0    0    0  739    0    0    0     0     0 # beta score > 90%
...
exportMMPFM(fun_output = K562_CEBPB, fun = "searchMotif", save_motif_PFM = T, save_betaScore_matrix = T)
#> Start exporting ... ...
...
#> if there is any peak loaded from TFregulome
#> Loading target peak list ... ...
#> ... You have 3 TFBS(s) requested to be loaded from TFregulome server
#> ... You chose to load TF peaks with motif only. Using 'motif_only_for_target_peak' tunes your options
#> ... loading TFBS(s) from TFregulome now
#> ... ... peak file loaded successfully for id 'MM1_HSA_HeLa-S3_MAFF'
#> ... ... peak file loaded successfully for id 'MM1_HSA_HepG2_MAFF'
#> ... ... peak file loaded successfully for id 'MM1_HSA_K562_MAFF'
#> ... Done loading TFBS(s) from TFregulome
#> Loading compared peak list ... ...
#> ... You have 3 TFBS(s) requested to be loaded from TFregulome server
#> ... You chose to load TF peaks with motif only. Using 'motif_only_for_compared_peak' tunes your options
...
#> ... ... ... Both common peak sets and methylation profiles will be stored in a list, and named with 
#> 'common_peak_list' and 'methylation_profile' in the list. Use 'names()' in the output for its detials.
#> ... ... You chose to save MethMotif logo in PDF if any;
#> ... ... ... You chose entropy logo;
#> ... ... ... You chose to show all methylation levels;
...
```{r echo=FALSE, fig.cap="Figure 10. MethMotif logos of MAFF common peaks in three cell types", out.width = '100%', fig.align="center"}
...
# exclusivePeaks
K562_MAFF_exclusive_output <- exclusivePeaks(target_peak_id = c("MM1_HSA_K562_MAFF"),
                                             motif_only_for_target_peak = T,
                                             excluded_peak_id = c("MM1_HSA_HeLa-S3_MAFF",
                                                                  "MM1_HSA_HepG2_MAFF"),
                                             motif_only_for_excluded_peak = T)
#> TFregulomeR::exclusivePeaks() starting ... ...
#> You chose to profile the methylation levels in 200bp window around peak summits, if there is any peak loaded from TFregulome
#> Loading target peak list ... ...
#> ... You have 1 TFBS(s) requested to be loaded from TFregulome server
#> ... You chose to load TF peaks with motif only. Using 'motif_only_for_target_peak' tunes your options
#> ... loading TFBS(s) from TFregulome now
...
#> ... Done loading TFBS(s) from TFregulome
#> Loading excluded peak list ... ...
#> ... You have 2 TFBS(s) requested to be loaded from TFregulome server
#> ... You chose to load TF peaks with motif only. Using 'motif_only_for_excluded_peak' tunes your options
#> ... loading TFBS(s) from TFregulome now
#> ... ... peak file loaded successfully for id 'MM1_HSA_HeLa-S3_MAFF'
#> ... ... peak file loaded successfully for id 'MM1_HSA_HepG2_MAFF'
...
HeLa_MAFF_exclusive_output <- exclusivePeaks(target_peak_id = c("MM1_HSA_HeLa-S3_MAFF"),
                                             motif_only_for_target_peak = T,
...
HepG2_MAFF_exclusive_output <- exclusivePeaks(target_peak_id = c("MM1_HSA_HepG2_MAFF"),
                                             motif_only_for_target_peak = T,
                                             excluded_peak_id = c("MM1_HSA_K562_MAFF",
...
K562_MAFF_exclusivePeaks <- K562_MAFF_exclusive_result$exclusive_peak_list$MM1_HSA_K562_MAFF_exclusive_peaks
...
HeLa_MAFF_exclusivePeaks <- HeLa_MAFF_exclusive_result$exclusive_peak_list$`MM1_HSA_HeLa-S3_MAFF_exclusive_peaks`
...
HepG2_MAFF_exclusivePeaks <- HepG2_MAFF_exclusive_result$exclusive_peak_list$MM1_HSA_HepG2_MAFF_exclusive_peaks
```

```{r echo=FALSE, fig.cap="Figure 11. MethMotif logos of MAFF exclusive peaks in three cell types", out.width = '100%', fig.align="center"}
knitr::include_graphics("../inst/extdata/MethMotif_logos_of_MAFF_exclusive.png")
```

We observed distinct MAFF motifs in MAFF exclusive peaks across three cell types (Figure 11). In K562, sequence TGA is highly enriched adjacent to TCAGCA, while in HepG2 the MAFF motif in the cell speccific regions is only TCAGCA. The MAFF motif in HeLa-S3 is between K562 and HepG2. Notably, distinct MAFF motifs are associated with different DNA methylation profiles across cell types. In K562 MAFF exclusive binding sites, the CpG is hypomethylated, while it can be methylated or unmethylated in HepG2 MAFF exclusive binding sites. Further, we noticed that different cell-specific MAFF target regions lead to distinct gene ontology enrichments analysed as below.
...
K562_MAFF_excl_IPM_output <- intersectPeakMatrix(user_peak_list_x = list(K562_MAFF_exclusivePeaks),
...
#> Loading peak list y ... ...
#> ... You have 108 TFBS(s) requested to be loaded from TFregulome server
#> ... You chose to load TF peaks regardless of presence of motif. Using 'motif_only_for_id_y' tunes your options
#> ... loading TFBS(s) from TFregulome now
#> .. ... peak file loaded successfully for id 'MM1_HSA_K562_AFF1'
#> .. ... peak file loaded successfully for id 'MM1_HSA_K562_ARID2'
#> .. ... peak file loaded successfully for id 'MM1_HSA_K562_ARID3A'
...
HeLa_MAFF_excl_IPM_output <- intersectPeakMatrix(user_peak_list_x = list(HeLa_MAFF_exclusivePeaks), 
                                                 user_peak_x_id = c("MM1_HSA_HeLa-S3_MAFF"),
...
#> ... You have 1 customised peak set(s)
#> Loading peak list y ... ...
#> ... You have 31 TFBS(s) requested to be loaded from TFregulome server
#> ... You chose to load TF peaks regardless of presence of motif. Using 'motif_only_for_id_y' tunes your options
#> ... loading TFBS(s) from TFregulome now
#> .. ... peak file loaded successfully for id 'MM1_HSA_HeLa-S3_CEBPB'
#> .. ... peak file loaded successfully for id 'MM1_HSA_HeLa-S3_CTCF'
#>        ... ...
#> ... Done loading TFBS(s) from TFregulome
...
HepG2_MAFF_excl_IPM_output <- intersectPeakMatrix(user_peak_list_x = list(HepG2_MAFF_exclusivePeaks), 
                                                  user_peak_x_id = c("MM1_HSA_HepG2_MAFF"), 
...
#> ... You have 1 customised peak set(s)
#> Loading peak list y ... ...
#> ... You have 66 TFBS(s) requested to be loaded from TFregulome server
#> ... You chose to load TF peaks regardless of presence of motif. Using 'motif_only_for_id_y' tunes your options
#> ... loading TFBS(s) from TFregulome now
#> .. ... peak file loaded successfully for id 'MM1_HSA_HepG2_ARID3A'
#> .. ... peak file loaded successfully for id 'MM1_HSA_HepG2_ATF2'
#>        ... ...
#> ... Done loading TFBS(s) from TFregulome
...
K562_MAFF_excl_IPM_result_t <- as.data.frame(t(K562_MAFF_excl_IPM_result$intersection_matrix))
...
HeLa_MAFF_excl_IPM_result_t <- as.data.frame(t(HeLa_MAFF_excl_IPM_result$intersection_matrix))
...
HepG2_MAFF_excl_IPM_result_t <- as.data.frame(t(HepG2_MAFF_excl_IPM_result$intersection_matrix))
...
```{r echo=FALSE, fig.cap="Figure 12. TF co-binding profiles around MAFF exclusive peaks in three cell types", out.width = '100%', fig.align="center"}
knitr::include_graphics("../inst/extdata/co-binding_of_MAFF_exclusive_peaks.png")
```

Interestingly, we observed several TFs co-binding with MAFF in MAFF cell specific loci in K562, not only incluing small maf protein MAFK, but also many Jun-related factors such as NFE2, NFE2L2 and JUND (Figure 12). However, in HepG2, only MAFK shows a high co-occupancy. We hypothesise the high enrichment of sequence TGA in the motif of K562 MAFF exclusive peaks is contributed by the Jun-related factors. To validate it, we further focused on the subset of K562 MAFF exclusive peaks which are bound by two very high co-binding Jun-related factors, NFE2 and NFE2L2, using `exclusivePeaks`.

```{r eval=FALSE}
# exclude the co-binding regions with NFE2 and NFE2L2 from K562 MAFF exclusive peaks 

K562_MAFF_exclPeaks_NoNFE_output <- exclusivePeaks(user_target_peak_list = list(K562_MAFF_exclusivePeaks), 
                                                   user_target_peak_id = c("MM1_HSA_K562_MAFF"),
...
#> Loading excluded peak list ... ...
#> ... You have 2 TFBS(s) requested to be loaded from TFregulome server
#> ... You chose to load TF peaks regardless of presence of motif. Using 'motif_only_for_excluded_peak' tunes your options
#> ... loading TFBS(s) from TFregulome now
#> .. ... peak file loaded successfully for id 'MM1_HSA_K562_NFE2'
#> .. ... peak file loaded successfully for id 'MM1_HSA_K562_NFE2L2'
#> ... Done loading TFBS(s) from TFregulome
#> Start analysing: MM1_HSA_K562_MAFF... ...
#> Done analysing.
...
#> ... ... You chose NOT to return methylation profile;
#> ... ... You chose NOT to return exclusive peak summary;
#> ... ... ... Only exclusive peak sets will be returned in a list and named with 'exclusive_peak_list'.
#> ... ... You chose to save MethMotif logo in PDF if any;
#> ... ... ... You chose entropy logo;
#> ... ... ... You chose to show all methylation levels;
#> Success: a PDF named 'MM1_HSA_K562_MAFF_exclusive_peaks-logo-entropy.pdf' has been saved!
K562_MAFF_exclPeaks_NoNFE_peaks <- K562_MAFF_exclPeaks_NoNFE_result$exclusive_peak_list$MM1_HSA_K562_MAFF_exclusive_peaks
...
K562_MAFF_excl_NoNFE_IPM_output <- intersectPeakMatrix(user_peak_list_x = list(K562_MAFF_exclPeaks_NoNFE_peaks), 
                                                       user_peak_x_id = c("MM1_HSA_K562_MAFF"), 
                                                       peak_id_y = K562_TFBS$ID)
#> You chose NOT to profile the methylation levels in 200bp window around peak summits
#> Loading peak list x ... ...
#> ... You have 1 customised peak set(s)
...
K562_MAFF_excl_NoNFE_IPM_result_t <- as.data.frame(t(K562_MAFF_excl_NoNFE_IPM_result$intersection_matrix))
...
```{r echo=FALSE, fig.cap="Figure 13. TF co-binding profiles around K562 MAFF exclusive peaks excluded NFE2 and NFE2L2 peaks", out.width = '100%', fig.align="center"}
knitr::include_graphics("../inst/extdata/co-binding_of_K562_MAFF_exclusive_peaks_no_NFE2_NFE2L2.png")
```

After removing co-binding regions with NFE2 and NFE2L2, a significant decrease in enrichment of sequence TGA in the motif of K562-specific MAFF binding sites (Figure 13). Indeed, the distinct MAFF co-binding partners result in the varied motifs enriched in MAFF peaks across different cell types. In addition, the DNA sequences can be hypermethylated in the HepG2 MAFF exclusive peaks, in which only small maf proteins are co-occupying, while the K562 specific MAFF binding loci are co-bound by many Jun-related factors and hypomethylated (Figure 11). Therefore, it is appealing to hypothesise that different DNA methylation levels in MAFF binding sites are associated with different binding partners.
