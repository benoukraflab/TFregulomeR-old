---
title: "TFregulomeR: Flexible characterisation of transcription factor binding partners and binding motifs using cell-specific cistrome and methylome meta-data"
author: 
...
output: 
  rmarkdown::html_document:
    ...
    fig_caption: yes
vignette: >
  %\VignetteIndexEntry{TFregulomeR: Flexible characterisation of transcription factor binding partners and binding motifs using cell-specific cistrome and methylome meta-data}
...
searching, including the information in TFregulomeR ID (important for following analysis), species, 
...
We have designed some useful functions to aid in data retrival from TFregulomeR data warehouse. 
...
provided peak sets. It should be noted that if the customised peak set is a subset of a TFregulomeR 
peak set, it's highly recommended that its TFregulomeR ID should be provided correspondingly in 
`user_target_peak_id` if one opts to profile the DNA methylation levels for the peaks. Even though TFregulomeR peak sets 
...
During the analyisis, each target peak set from `target_peak_id` (will be automatically loaded from TFregulomeR warehouse during the analysis) and 
...
for each common peak subset, whose TF source in TFregulomeR warehouse is MethMotif, in `target_peak_id` and `user_target_peak_list` tagged with TFregulomeR IDs in `user_target_peak_id`.
...
# common with (conserved in) the CEBPB peaks in all cell types from TFregulomeR warehouse,
...
MethMotif in TFregulomeR warehouse and 4) the (Meth)Motif logo if TF input is from TFregulomeR warehouse or with TFregulomeR ID.
...
# customised input peaks are not orginated from TFregulomeR warehouse, so no DNA methylation states
...
If `methylation_profile_in_narrow_region=T`, DNA methylation profiling in +/- 100bp surrounding peak summit will be performed 
for each unique peak subset, whose TF source in TFregulomeR warehouse is MethMotif, in `target_peak_id` and `user_target_peak_list` tagged with TFregulomeR IDs in `user_target_peak_id`.
...
# 2) All CEBPB TFregulomeR ID except MM1_HSA_K562_CEBPB
...
MethMotif in TFregulomeR warehouse and 4) the (Meth)Motif logo if input is from TFregulomeR warehouse or with TFregulomeR ID.
...
each customised peak set in `user_peak_x_id`. If the customised peak set is a TFregulomeR TF subset, it's highly
...
if `methylation_profile_in_narrow_region=T`. Even though TFregulomeR peak sets 
...
Similarly, when `save_MethMotif_logo` option is wanted and `angle_of_logo = "x"`, the function will plot and save the (Meth)Motif logos for the intersected TFBSs from `peak_list_x` (if any is originally from TFregulomeR warehouse). Indeed, it's not necessary to plot all (Meth)Motif logos for every pair of intersection at the same time. One can focus only on some subsets of `peak_list_x` and `peak_list_y`, using `saving_MethMotif_logo_x_id` and `saving_MethMotif_logo_y_id` respectively.
...
TFregulomeR also allows uers to plot the distributions of a given TFBS from the TFregulomeR warehouse in a list of peak sets, using `motifDistrib` and `plotDistrib` sequentially. By providing the TFregulomeR ID in `id` as the input of the `motifDistrib`, `motifDistrib` will calculate the occurrences the TFBSs in the given list of peak sets input in `peak_list`. The unique IDs correpsonding to `peak_list` is required to be provided at the same time using `peak_id`. If a peak set is a TFregulomeR TF subset, the TFregulomeR ID should be input accordingly; if it is self provided, you can name it with a unique ID yourself. 

It should be noted that even though the loaded peak regions from the TFregulomeR warehouse are the peak summits, you don't need to expand the regions. Once you tell `motifDistrib` the peak set is a TFregulomeR TF subset by providing TFregulomeR ID in the `peak_id`, it will automatically operate on the peaks itself.
